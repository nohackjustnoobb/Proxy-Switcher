...
class SocksProxy(BaseRequestHandler):
    ...
    def print(self, msg):
        logger.debug(
            f'client "{self.client_address[0]}:{self.client_address[1]}": {msg}'
        )
    ...
    def handle(self):
        ...
        try:
            ...
            remote = proxy.socket() if proxy else socket.socket()
            try:
...
class Proxy:
    ...
    def restart(self):
        ...
        self.isDown = True
        logger.info(f"{self.address} is down")
    ...
    def try_restart(self):
        while True:
            ...
            # restart the proxy
            if self.restartCMD:
                logger.info(f"Restarting {self.address}...")
                ...
                if result.returncode != 0:
                    logger.error(f"{self.address} failed to restart: {result.stderr}")

            # timeout
            time.sleep(10)

        self.isDown = False
        logger.info(f"{self.address} is up")
    ...
    def test(self):
        with concurrent.futures.ThreadPoolExecutor() as executor:
            return False not in set(executor.map(self.test_link, testLinks))


class ProxyManager:
    proxies = []

    def register(self, address, restartCMD):
        logger.info(f"Registering {address}")
        self.proxies.append(Proxy(address, restartCMD))

    def get(self):
        filtered = list(filter(lambda x: not x.isDown, self.proxies))
        return random.choice(filtered) if len(filtered) else None
...
for proxy in config.get("proxies", []):
    proxyManager.register(proxy["address"], proxy.get("restartCMD", None))
